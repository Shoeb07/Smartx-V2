...
import { ArrowUpRight, Globe, Smartphone, Clock, ExternalLink } from 'lucide-react'
...
const statusBadge = {
  live: { label: '● Live', color: '#00e5b0', bg: 'rgba(0,229,176,0.1)', border: 'rgba(0,229,176,0.2)' },
  dev: { label: '◐ In Dev', color: '#f59e0b', bg: 'rgba(245,158,11,0.1)', border: 'rgba(245,158,11,0.2)' },
  open: { label: '✦ Open Slot', color: '#6c63ff', bg: 'rgba(108,99,255,0.1)', border: 'rgba(108,99,255,0.2)' },
}
    ...
    <section id="work" className="py-32 bg-[#0a0a10]" ref={ref}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-16">
          ...
          >
            Every project we take on gets our full cinematic treatment — no templates, no shortcuts.
          </motion.p>
        </div>
          ...
          {projects.map((proj, i) => {
            ...
            return (
              <motion.article
                ...
                animate={inView ? { opacity: 1, y: 0 } : {}}
                transition={{ delay: 0.1 + i * 0.11, duration: 0.75, ease: [0.22, 1, 0.36, 1] }}
              ...
              >
                {/* Gradient sweep */}
                <div className={`absolute inset-0 bg-gradient-to-br ${proj.bg} opacity-70 pointer-events-none`} />
                {/* Hover glow */}
                <div
                  className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-3xl"
                  style={{ background: `radial-gradient(ellipse at 20% 20%, ${proj.accent}0d 0%, transparent 65%)` }}
                />
                ...
                <div className="relative z-10">
                  ...
                  <div
                    ...
                    style={{ background: `${proj.accent}0a`, border: `1px solid ${proj.accent}18` }}
                  >
                    {/* Subtle grid inside card */}
                    <div className="absolute inset-0 opacity-30"
                      style={{ backgroundImage: `repeating-linear-gradient(0deg, transparent, transparent 24px, ${proj.accent}08 24px, ${proj.accent}08 25px), repeating-linear-gradient(90deg, transparent, transparent 24px, ${proj.accent}08 24px, ${proj.accent}08 25px)` }}
                    />
                    <div className="relative text-center">
                      ...
                      <p className="text-[11px] text-white/20 tracking-wider uppercase">{proj.title}</p>
                    </div>
                    {/* Type icons */}
                    <div className="absolute top-3 right-3 flex gap-1.5">
                      {(proj.type === 'web' || proj.type === 'both') && <Globe size={11} className="text-white/20" />}
                      {(proj.type === 'mobile' || proj.type === 'both') && <Smartphone size={11} className="text-white/20" />}
                    </div>
                  </div>

                  {/* Title & desc */}
                  <h3 className="font-syne font-bold text-xl mb-2 group-hover:text-white transition-colors">{proj.title}</h3>
                  <p className="text-white/45 text-[14px] leading-relaxed mb-5">{proj.description}</p>

                  {/* Highlights */}
                  <ul className="space-y-1.5 mb-5">
                    {proj.highlights.map((h) => (
                      <li key={h} className="flex items-start gap-2 text-[12px] text-white/35">
                        <span className="w-1 h-1 rounded-full flex-shrink-0 mt-1.5" style={{ background: proj.accent }} />
                        {h}
                      </li>
                    ))}
                  </ul>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
                    <span className="text-[12px] font-medium" style={{ color: proj.resultColor }}>
                      {proj.result}
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {proj.tags.map((t) => (
                        <span key={t} className="text-[10px] text-white/25 border border-white/[0.06] px-2 py-0.5 rounded">
                          {t}
                        </span>
                      ))}
                    </div>
        ...
        >
          More case studies coming soon as projects go live. Every story we build is worth telling.
        </motion.p>
      </div>
    </section>
  )
